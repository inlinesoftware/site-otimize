...
window.onload = function () {
  setTimeout(() => {
    const formWhatsapp = document.getElementById("rd-button-l1jpanuf")
  
    console.log(formWhatsapp)
  
    formWhatsapp.addEventListener('click', () => {
      // Para contato whatsap
      fbq('track', 'Contact')
    })
...
setTimeout(() => {
  const calendlyWidget = document.getElementsByClassName('calendly-badge-widget')[0]

  console.log(calendlyWidget)

  calendlyWidget.addEventListener('click', () => {
    // Para agendamento
    fbq('track', 'Schedule')
  })
}, 7000)
